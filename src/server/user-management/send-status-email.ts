...
function getReactivatedEmailHtml(fullName: string): string {
	return `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin:0;padding:0;background-color:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:520px;margin:40px auto;background:#ffffff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden;">
    <div style="background:linear-gradient(135deg,#1C402E,#0f766e);padding:32px 24px;text-align:center;">
      <h1 style="color:#ffffff;font-size:22px;margin:0;">Account Reactivated</h1>
    </div>
    <div style="padding:32px 24px;">
      <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">
        Hi <strong>${fullName}</strong>,
      ...
      <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">
        Your DocuLens account has been <strong style="color:#0f766e;">reactivated</strong> by an administrator. You can now sign in again and continue using the platform.
      </p>
      <div style="text-align:center;margin:24px 0;">
        <a href="${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/login"
           style="display:inline-block;background:#1C402E;color:#ffffff;padding:12px 32px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px;">
      ...
      <p style="color:#6b7280;font-size:13px;line-height:1.5;margin:0;">
        If you did not request access to be restored, please contact your system administrator.
      </p>
    </div>
    <div style="background:#f9fafb;padding:16px 24px;text-align:center;border-top:1px solid #e5e7eb;">
      <p style="color:#9ca3af;font-size:12px;margin:0;">© ${new Date().getFullYear()} DocuLens. All rights reserved.</p>
...
export async function sendStatusEmail({
	...
	to: string;
	fullName: string;
	status: "approved" | "rejected" | "reactivated";
}) {
	const isApproved = status === "approved";
	const isReactivated = status === "reactivated";
	...
	const { data, error } = await resend.emails.send({
		...
		subject: isApproved
			...
			: isReactivated
				? "Your DocuLens Account Has Been Reactivated"
				: "DocuLens Account Update",
		html: isApproved
			? getApprovedEmailHtml(fullName)
			: isReactivated
				? getReactivatedEmailHtml(fullName)
