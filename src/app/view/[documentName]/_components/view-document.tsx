...
import { ChevronLeft, Bookmark, Share2, FileDown, Flag, BookText, Files, History, CalendarDays } from "lucide-react";
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-12 gap-8">
        <div className="col-span-12 lg:col-span-3">
          ...
          <div className="mt-4 space-y-3 text-sm">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {doc.tags.map((t) => (
                <div key={t} className="bg-blue-50 text-blue-700 px-2.5 py-1 rounded-full font-medium">{t}</div>
              ))}
            </div>
          </div>
          <div className="mt-6 border-t pt-4 space-y-2">
            <button className="w-full text-left bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium py-2 px-3 rounded-md flex items-center gap-3">
        ...
        <div className="col-span-12 lg:col-span-6 bg-white p-6 rounded-xl shadow-md border border-slate-200">
          ...
          <p className="text-sm text-slate-500 mb-4">Generate summary or ask questions about the content.</p>
          ...
          <div className="mt-6 border-t pt-6">
            <h4 className="font-bold text-slate-800 mb-3">Ask a question about the document</h4>
            <div className="flex gap-2">
              ...
              />
              <button className="bg-slate-700 text-white font-semibold py-2 px-4 rounded-lg hover:bg-slate-800">Ask</button>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-3 space-y-6">
          <div className="bg-white p-4 rounded-xl shadow-sm border">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="text-sm bg-slate-100 rounded-lg p-1 flex">
                <button className="px-2 py-0.5 rounded-md bg-white shadow text-xs font-semibold">Text</button>
                <button className="px-2 py-0.5 rounded-md text-slate-600 text-xs">PDF</button>
              </div>
            </div>
            <div className="text-xs text-slate-600 leading-relaxed max-h-48 overflow-y-auto p-2 bg-slate-50 rounded-md">
              ...
              <br />
              {doc.contentText || "The Department of Education (DepEd) is committed to providing a learning environment that is safe..."}
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border">
            ...
            <div className="space-y-3">
              {similar.map((s) => (
                <Link key={s.id} href={`/view/${s.slug}`} className="block text-xs text-blue-600 hover:bg-slate-50 p-2 rounded-md">
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border">
            <h4 className="font-bold text-slate-800 mb-2">Attachments & History</h4>
            <div className="text-xs space-y-2">
              <span className="flex items-center gap-2 text-slate-600"><Files className="h-4 w-4" /> Enclosure 1: School Calendar.pdf</span>
              <span className="flex items-center gap-2 text-slate-600"><History className="h-4 w-4" /> Version History (3)</span>
            </div>
          </div>
        </div>
      </div>
